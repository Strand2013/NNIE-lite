...
typedef struct {
    ...
    hi_u16  detail_smooth_str[ISP_AUTO_ISO_STRENGTH_NUM];
} hi_isp_cmos_demosaic;

typedef struct {
    hi_bool  enable;
    hi_bool  low_power_enable;
    ...
    hi_u16   coarse_str[ISP_BAYER_CHN_NUM][ISP_AUTO_ISO_STRENGTH_NUM];             // u10.0
...
typedef struct {
    ...
    hi_u16   short_check_thd;   /* RW;range:[0x0,0xFF];format:8.0 */
    ...
    hi_u8    g_sigma_gain[3];
    hi_u8    rb_sigma_gain[3];
    hi_u8    mdt_still_thd;     /* RW;range:[0x0,0xFE];format:8.0 */
    hi_u8    mdt_full_thd;      /* RW;range:[0x0,0xFE];format:8.0,only used for hi3559_av100 */
    hi_u8    mdt_long_blend;    /* RW;range:[0x0,0xFE] */
    hi_u8    full_mdt_sig_wgt;
    hi_u8    noise_floor[NoiseSet_EleNum];
} hi_isp_cmos_wdr;
...
typedef struct {
    hi_u16  gamma_lut[GAMMA_NODE_NUMBER];
} hi_isp_cmos_gamma;
...
typedef struct {
    ...
    hi_isp_rlsc_cabli_table lsc_calib_table[3];
} hi_isp_cmos_rlsc;

typedef struct {
    hi_bool   enable;
    hi_u32    y_ratio_lut[HI_ISP_CA_YRATIO_LUT_LENGTH];  // 1.10bit  Y Ratio For UV ; Max = 2047 FW Limit
    hi_s16    iso_ratio[ISP_AUTO_ISO_STRENGTH_NUM];     // 1.10bit  ISO Ratio  For UV,Max = 2047 FW Limi
    hi_bool   cp_enable;
    hi_u32    y_ratio_luty[HI_ISP_CA_YRATIO_LUT_LENGTH];
    hi_u32    y_ratio_lutu[HI_ISP_CA_YRATIO_LUT_LENGTH];
    hi_u32    y_ratio_lutv[HI_ISP_CA_YRATIO_LUT_LENGTH];
} hi_isp_cmos_ca;
...
typedef struct {
    hi_u8  x;                     /* RW;Range:[0x0,0x81];Format:8.0;The X point of the knee */
    hi_u16 y;                    /* RW;Range:[0x0,0x8000];Format:16.0;The Y point of the  knee */
} hi_isp_cmos_split_point;

typedef struct {
    hi_bool enable;             /* RW;Range:[0x0,0x1];Format:1.0; */
    hi_u8   input_width_sel;     /* RW;Range:[0x0,0x3];Format:2.0;Inputwidthselect: 0=12bit; 1=14bit; 2=16bit; */
    hi_u8   mode_in;            /* RW;Range:[0x0,0x3];Format:2.0;ModeIn: 0=linear; 2=16LOG; 3=sensor-built-in */
    hi_u8   mode_out;           /* RW;Range:[0x0,0x3];Format:2.0;ModeOut: 0= 16bit when decompress; 1=2chn ; 2=3chn; */
    hi_u32  bit_depth_out;      /* RW;Range:[0xC,0x14];Format:5.0;The Bit depth of output */
    hi_isp_cmos_split_point split_point[ISP_SPLIT_POINT_NUM];
} hi_isp_cmos_split;
...
typedef struct {
    hi_u32 pre_log_lut[PRE_LOG_LUT_SIZE];
} hi_isp_cmos_preloglut;

typedef struct {
    hi_u32 log_lut[LOG_LUT_SIZE];
} hi_isp_cmos_loglut;
...
typedef struct {
    hi_isp_awb_gain_switch awb_gain_switch;
} hi_isp_cmos_awb_attr;
...
typedef union {
    ...
    struct {
        ...
        hi_u64  bit1_ldci             : 1 ;   /* [6] */
        hi_u64  bit1_dpc              : 1 ;   /* [7] */
        hi_u64  bit1_lsc              : 1 ;   /* [8] */
        hi_u64  bit1_r_lsc            : 1 ;   /* [9] */
        hi_u64  bit1_ge               : 1 ;   /* [10] */
        hi_u64  bit1_anti_false_color : 1 ;   /* [11] */
        hi_u64  bit1_bayer_nr         : 1 ;   /* [12] */
        hi_u64  bit1_split            : 1 ;   /* [13] */
        hi_u64  bit1_ca               : 1 ;   /* [14] */
        hi_u64  bit1_clut             : 1 ;   /* [15] */
        hi_u64  bit1_log_lut          : 1 ;   /* [16] */
        hi_u64  bit1_pre_log_lut      : 1 ;   /* [17] */
        hi_u64  bit1_wdr              : 1 ;   /* [18] */
        hi_u64  bit1_awb              : 1 ;   /* [19] */
        hi_u64  bit44_rsv             : 44;   /* [20:63] */
    };
} hi_isp_cmos_alg_key;
